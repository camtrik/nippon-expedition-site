...
其他可选参考：工坊描述双语成稿 `../nippon_expedition/steam_description_{zh,en}.txt`（大段介绍已有现成译文，首发条目就是从这里提炼的）；子 mod 说明 `../nippon_expedition_{nrs,cathay,yinyin,wuh}_compat/README_{zh,en}.md`（都是作者定稿的双语成文，直接搬）。
...
- `date` 可选。**还没上工坊就留空**——该条会排在所有有日期的之上，页面上不显示日期。别编一个假日期。真发布了再补。留空的条目文件名也不带日期，叫 `v<版本>.md`。
